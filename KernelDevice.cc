...
int KernelDevice::open(const string &p)
{
	...
	r = posix_fadvise(fd_buffereds[WRITE_LIFE_NOT_SET], 0, 0,
					  POSIX_FADV_RANDOM);
	if (r) {
		r = -r;
	...
	dout(1)
		<< __func__ << " size " << size << " (0x" << std::hex << size
...
void KernelDevice::_detect_vdo()
{
	...
	if (vdo_fd >= 0) {
		dout(1) << __func__ << " VDO volume " << vdo_name << " maps to "
				<< devname << dendl;
	} else {
	...
	if (aio) {
		...
		if (r < 0) {
			if (r == -EAGAIN) {
				...
				derr << __func__ << " io_setup(2) failed: " << cpp_strerror(r)
					 << dendl;
			}
			return r;
		}
...
void KernelDevice::_aio_thread()
{
	...
	while (!aio_stop) {
		...
		aio_t *aio[max];
		int r = io_queue->get_next_completed(cct->_conf->bdev_aio_poll_ms, aio,
											 max);
		if (r < 0) {
			...
			for (int i = 0; i < r; ++i) {
				...
				if (r < 0) {
					...
					derr << "aio to 0x" << std::hex << aio[i]->offset << "~"
						 << aio[i]->length << std::dec << " but returned: " << r
						 << dendl;
					ceph_abort_msg(
						"unexpected aio return value: does not match length");
				}

				dout(10)
					<< __func__ << " finished aio " << aio[i] << " r " << r
					<< " ioc " << ioc << " with "
					...
					if (cct->_conf->bdev_debug_aio_suicide_timeout) {
						...
						if (debug_stall_since < cutoff) {
							derr << __func__ << " stalled aio " << debug_oldest
								 << " since " << debug_stall_since
								 << ", timeout is "
								 << cct->_conf->bdev_debug_aio_suicide_timeout
								 << "s, suicide" << dendl;
							ceph_abort_msg(
								"stalled aio... buggy kernel or bad device?");
						}
					}
				}
			}
	...
	if (cct->_conf->bdev_debug_inflight_ios) {
		...
		if (debug_inflight.intersects(offset, length)) {
			derr << __func__ << " inflight overlap of 0x" << std::hex << offset
				 << "~" << length << std::dec << " with " << debug_inflight
				 << dendl;
			ceph_abort();
		}
...
{
	dout(20)
		<< __func__ << " ioc " << ioc << " pending " << ioc->num_pending.load()
							  ...
							  int write_hint)
{
	...
	size_t idx = 0;
	do {
		auto r = ::pwritev(choose_fd(buffered, write_hint), &iov[idx],
						   iov.size() - idx, o);
	...
	if (aio && dio && !buffered) {
		...
		if (cct->_conf->bdev_inject_crash &&
			...
			ioc->pending_aios.push_back(
				aio_t(ioc, choose_fd(false, write_hint)));
			++ioc->num_pending;
	...
	if (buffered) {
		...
		if (mono_clock::now() - start1 >= make_timespan(age)) {
			derr << __func__ << " stalled read "
				 ...
				 << " (buffered) since " << start1 << ", timeout is " << age
				 << "s" << dendl;
		}
	} else {
		...
		if (mono_clock::now() - start1 >= make_timespan(age)) {
			derr << __func__ << " stalled read "
				 << " 0x" << std::hex << off << "~" << len << std::dec
				 << " (direct) since " << start1 << ", timeout is " << age
				 << "s" << dendl;
		}
		if (r < 0) {
			r = -errno;
