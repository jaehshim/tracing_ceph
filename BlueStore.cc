...
static void get_object_key(CephContext *cct, const ghobject_t &oid, S *key)
{
	...
	size_t max_len =
		ENCODED_KEY_PREFIX_LEN + (oid.hobj.nspace.length() * 3 + 1) +
...
void _dump_extent_map(CephContext *cct, const BlueStore::ExtentMap &em)
{
	...
	for (auto &e : em.extent_map) {
		...
		if (blob.has_csum()) {
			...
			dout(LogLevelV) << __func__ << "      csum: " << std::hex << v
							<< std::dec << dendl;
		}
		std::lock_guard l(e.blob->shared_blob->get_cache()->lock);
		for (auto &i : e.blob->shared_blob->bc.buffer_map) {
			dout(LogLevelV)
				<< __func__ << "       0x" << std::hex << i.first << "~"
				<< i.second->length << std::dec << " " << *i.second << dendl;
		}
	}
}
...
public:
	SimpleCollectionListIterator(CephContext *cct,
								 const KeyValueDB::Iterator &it)
	...
	uint64_t min_alloc_size)
{
	...
	for (auto it = extent_map.seek_lextent(lookup_start_offset);
		...
		dout(30)
			<< __func__ << " " << *it << "alloc_units: " << alloc_unit_start
		...
		if (it->logical_offset >= start_touch_offset &&
			...
			if (!b->get_blob().is_compressed()) {
				if (blob_info_counted && used_alloc_unit == alloc_unit_start) {
					--blob_info_counted
						  ->expected_allocations; // don't need to allocate
						// new AU for compressed
						// data since another
						// collocated uncompressed
						// blob already exists
					dout(30) << __func__ << " --expected:" << alloc_unit_start
							 << dendl;
				}
			...
			int adjust =
				(used_alloc_unit && used_alloc_unit == alloc_unit_start) ? 0 :
																			 1;
			bi.expected_allocations +=
				alloc_unit_end - alloc_unit_start + adjust;
			dout(30) << __func__
			...
			dout(30) << __func__ << " affected_blob:" << *b << " unref 0x"
					 << std::hex << it->length << " referenced = 0x"
					 << bi.referenced_bytes << std::dec << dendl;
			...
			if (blob_info_counted && used_alloc_unit == alloc_unit_start) {
				...
				dout(30)
					<< __func__
					<< " --expected_allocations:" << alloc_unit_start << dendl;
			}
			used_alloc_unit = alloc_unit_end;
			blob_info_counted = nullptr;
		}
		...
		if (bi.referenced_bytes == 0) {
			...
			if (benefit >= g_conf()->bluestore_gc_enable_blob_threshold) {
				if (bi.collect_candidate) {
					...
					do {
						if (it->blob.get() == b) {
							extents_to_collect.insert(it->logical_offset,
													  it->length);
						}
	...
	const BlueStore::old_extent_map_t &old_extents, uint64_t min_alloc_size)
{
	...
	for (auto it = old_extents.begin(); it != old_extents.end(); ++it) {
		...
		if (b->get_blob().is_compressed()) {
			...
			gc_start_offset =
				min(gc_start_offset, (uint64_t)it->e.blob_start());
...
struct LruBufferCacheShard : public BlueStore::BufferCacheShard {
	...
	void _audit(const char *s) override
	{
		...
		if (s != buffer_bytes) {
			derr << __func__ << " buffer_size " << buffer_bytes << " actual "
				 << s << dendl;
			for (auto i = lru.begin(); i != lru.end(); ++i) {
...
public:
	...
	{
		dout(20)
			<< __func__ << " level " << level << " near " << near << " on "
		...
		if (near) {
			...
			case BUFFER_WARM_IN:
				...
				// do this.
				dout(20)
					<< __func__ << " move to front of warm " << *b << dendl;
				warm_in.push_front(*b);
				break;
			case BUFFER_WARM_OUT:
	...
	void _trim_to(uint64_t max) override
	{
		if (buffer_bytes > max) {
			...
			if (buffer_num) {
				...
				kout =
					calculated_num * cct->_conf->bluestore_2q_cache_kout_ratio;
			}
			...
			while (to_evict_bytes > 0) {
				...
				ceph_assert(b->is_clean());
				dout(20)
					<< __func__ << " buffer_warm_in -> out " << *b << dendl;
				ceph_assert(buffer_bytes >= b->length);
				buffer_bytes -= b->length;
			...
			if (evicted > 0) {
				dout(20)
					<< __func__ << " evicted " << byte_u_t(evicted)
	...
	void _audit(const char *s) override
	{
		...
		if (warm_in_bytes != list_bytes[BUFFER_WARM_IN]) {
			derr << __func__ << " warm_in_list_bytes "
				 << list_bytes[BUFFER_WARM_IN] << " != actual " << warm_in_bytes
				 << dendl;
			ceph_assert(warm_in_bytes == list_bytes[BUFFER_WARM_IN]);
		}

		if (s != buffer_bytes) {
			derr << __func__ << " buffer_bytes " << buffer_bytes << " actual "
				 << s << dendl;
			ceph_assert(s == buffer_bytes);
		}
									 ...
									 uint32_t length)
{
	...
	while (i != buffer_map.end()) {
		...
		if (b->offset < offset) {
			...
			if (b->end() > end) {
				...
				if (b->data.length()) {
					...
					_add_buffer(cache,
								new Buffer(this, b->state, b->seq, end, tail),
								0, b);
				}
				if (!b->is_writing()) {
		...
		if (b->data.length()) {
			...
			_add_buffer(cache, new Buffer(this, b->state, b->seq, end, keep), 0,
						b);
		}
		_rm_buffer(cache, i);
...
BlueStore::OnodeRef BlueStore::OnodeSpace::lookup(const ghobject_t &oid)
{
	...
	{
		std::lock_guard l(cache->lock);
		ceph::unordered_map<ghobject_t, OnodeRef>::iterator p =
			onode_map.find(oid);
		if (p == onode_map.end()) {
			...
			ldout(cache->cct, 30)
				...
				<< p->second->nref << " " << p->second->cached << " "
				<< p->second->pinned << dendl;
	...
	const mempool::bluestore_cache_meta::string &new_okey)
{
	std::lock_guard l(cache->lock);
	ldout(cache->cct, 30)
		<< __func__ << " " << old_oid << " -> " << new_oid << dendl;
	...
	if (pn != onode_map.end()) {
		ldout(cache->cct, 30)
			<< __func__ << "  removing target " << pn->second << dendl;
...
template <int LogLevelV = 30> void BlueStore::OnodeSpace::dump(CephContext *cct)
{
	for (auto &i : onode_map) {
		ldout(cct, LogLevelV)
			<< i.first << " : " << i.second << " " << i.second->nref << " "
			<< i.second->cached << " " << i.second->pinned << dendl;
	}
}
	...
	if (get_blob().is_compressed()) {
		...
		for (auto e : get_blob().get_extents()) {
			if (!e.is_valid()) {
				...
				shared_blob->bc.discard(shared_blob->get_cache(), pos,
										e.length);
			}
			pos += e.length;
		}
...
void BlueStore::ExtentMap::update(KeyValueDB::Transaction t, bool force)
{
	...
	dout(20)
		<< __func__ << " " << onode->oid << (force ? " force" : "") << dendl;
	if (onode->onode.extent_map_shards.empty()) {
		if (inline_bl.length() == 0) {
			...
			dout(20) << __func__ << "  inline shard " << len << " bytes from "
					 << n << " extents" << dendl;
			if (!force &&
		...
		while (p != shards.end()) {
			...
			if (p->dirty) {
				...
				if (encode_some(p->shard_info->offset,
								endoff - p->shard_info->offset, bl,
								&p->extents)) {
					if (force) {
						derr << __func__ << "  encode_some needs reshard"
							 << dendl;
						ceph_assert(!force);
					}
				}
				...
				if (!force) {
					...
					// avoid resharding the trailing shard, even if it is small
						...
						if (p == shards.begin()) {
							...
							if (prev_p->shard_info->bytes >
								n->shard_info->bytes) {
								request_reshard(p->shard_info->offset,
												endoff + 1);
							} else {
								request_reshard(prev_p->shard_info->offset,
												endoff);
							}
						}
					}
...
void BlueStore::ExtentMap::reshard(KeyValueDB *db, KeyValueDB::Transaction t)
{
	...
	for (auto &p : spanning_blob_map) {
		dout(20) << __func__ << "   spanning blob " << p.first << " "
				 << *p.second << dendl;
	}
	...
	if (!shards.empty()) {
		...
		dout(20)
			<< __func__ << "   shards [" << si_begin << "," << si_end << ")"
	...
	dout(20) << __func__ << "  extent_avg " << extent_avg << ", target "
			 << target << ", slop " << slop << dendl;
	...
	if (new_shard_info.empty() && (si_begin > 0 || si_end < shards.size())) {
		...
		dout(20)
			<< __func__ << "  new shard 0x" << std::hex << needs_reshard_begin
	...
	if (sv.empty()) {
		...
		sv.insert(sv.begin() + si_begin, new_shard_info.begin(),
				  new_shard_info.end());
		shards.insert(shards.begin() + si_begin, new_shard_info.size(),
					  Shard());
	...
	if (sv.empty()) {
		...
		dout(20)
			<< __func__ << " checking spanning blobs 0x[" << std::hex
		...
		decltype(
			onode->c->onode_map.cache->dumped_onodes)::value_type *oid_slot =
			nullptr;
		decltype(
			onode->c->onode_map.cache->dumped_onodes)::value_type *oldest_slot =
			nullptr;

		for (auto e = extent_map.lower_bound(dummy); e != extent_map.end();
			 ++e) {
			if (e->logical_offset >= needs_reshard_end) {
				break;
			}
			...
			if (e->blob_escapes_range(shard_start, shard_end - shard_start)) {
				if (!e->blob->is_spanning()) {
					...
					if (must_span) {
						...
						dout(20) << __func__ << "    adding spanning " << *b
								 << dendl;
						if (!was_too_many_blobs_check &&
							too_many_blobs_threshold &&
							spanning_blob_map.size() >=
								size_t(too_many_blobs_threshold)) {
							...
							for (size_t i = 0; i < dumped_onodes.size(); ++i) {
								...
								if (!oldest_slot ||
									(oldest_slot && dumped_onodes[i].second <
														oldest_slot->second)) {
				...
				if (e->blob->is_spanning()) {
					...
					e->blob->id = -1;
					dout(30)
						<< __func__ << "    un-spanning " << *e->blob << dendl;
				}
			}
		}
									   ...
									   bufferlist &bl, unsigned *pn)
{
	...
	for (auto p = start; p != extent_map.end() && p->logical_offset < end;
		...
		if (!must_reshard) {
			...
			p->blob->bound_encode(bound, struct_v,
								  p->blob->shared_blob->get_sbid(), false);
		}
	}
	if (must_reshard) {
		return true;
...

	{
		...
		for (auto p = start; p != extent_map.end() && p->logical_offset < end;
			...
			pos = p->logical_end();
			if (include_blob) {
				p->blob->encode(app, struct_v, p->blob->shared_blob->get_sbid(),
								false);
			}
		}
	}
				...
				[&](const string &final_key) {
					...
					if (r < 0) {
						derr << __func__ << " missing shard 0x" << std::hex
							 << p->shard_info->offset << std::dec << " for "
							 << onode->oid << dendl;
						ceph_assert(r >= 0);
					}
				});
	...
	while (start <= last) {
		...
		if (!p->loaded) {
			derr << __func__ << "on write 0x" << std::hex << offset << "~"
				 << length << " shard 0x" << p->shard_info->offset << std::dec
...
int BlueStore::ExtentMap::compress_extent_map(uint64_t offset, uint64_t length)
{
	...
	for (++n; n != extent_map.end(); p = n++) {
		...
		while (n != extent_map.end() && p->logical_end() == n->logical_offset &&
			...
			dout(20) << __func__ << " 0x" << std::hex << offset << "~" << length
					 << " next shard 0x" << shard_end << std::dec << " merging "
					 << *p << " and " << *n << dendl;
			p->length += n->length;
			rm(n++);
			++removed;
	...
	while (p != extent_map.end()) {
		...
		if (p->logical_offset < offset) {
			if (p->logical_end() > end) {
				...
				add(end, p->blob_offset + front + length,
					p->length - front - length, p->blob);
				p->length = front;
				break;
			} else {
				// deref tail
				...
				OldExtent *oe = OldExtent::create(c, offset,
												  p->blob_offset + keep,
...
BlueStore::ExtentMap::split_blob(BlobRef lb, uint32_t blob_offset, uint32_t pos)
{
	...
	dout(20)
		<< __func__ << " 0x" << std::hex << pos << " end 0x" << end_pos
		<< " blob_offset 0x" << blob_offset << std::dec << " " << *lb << dendl;
...
void BlueStore::Onode::flush()
{
	if (flushing_count.load()) {
		ldout(c->store->cct, 20)
			<< __func__ << " cnt:" << flushing_count << dendl;
		waiting_count++;
										   ...
										   uint64_t min_alloc_size)
{
	...
	for (auto w : writes) {
		if (b == w.b) {
			...
			auto loffs2_end =
				p2roundup(w.logical_offset + w.length0, min_alloc_size);
										...
										uint64_t length)
{
	...
	while (p != iomap.end()) {
		...
		if (end > offset + length) {
			...
			dout(20)
				<< __func__ << "  truncate front " << p->second.seq << " 0x"
				<< std::hex << p->first << "~" << p->second.bl.length()
				<< " drop_front 0x" << drop_front << " keep_tail 0x"
				<< keep_tail << " to 0x" << (offset + length) << "~"
				<< keep_tail << std::dec << dendl;
			auto &s = iomap[offset + length];
			s.seq = p->second.seq;
			s.bl.substr_of(p->second.bl, drop_front, keep_tail);
			i->second -= drop_front;
		} else {
			dout(20) << __func__ << "  drop " << p->second.seq << " 0x"
					 << std::hex << p->first << "~" << p->second.bl.length()
					 << std::dec << dendl;
...
void BlueStore::Collection::split_cache(Collection *dest)
{
	...
	while (p != onode_map.onode_map.end()) {
		...
		if (!p->second->oid.match(destbits, destpg.pgid.ps())) {
			...
			if (o->cached) {
				get_onode_cache()->move_pinned(dest->get_onode_cache(),
											   o.get());
			}
			o->c = dest;
			...
			for (auto sb : sbvec) {
				...
				ldout(store->cct, 20)
					<< __func__ << "  moving " << *sb << dendl;
				if (sb->get_sbid()) {
					ldout(store->cct, 20)
						<< __func__ << "   moving registration " << *sb
						<< dendl;
					shared_blob_set.remove(sb);
...
void *BlueStore::MempoolThread::entry()
{
	...
	if (store->cache_autotune && binned_kv_cache != nullptr) {
		pcm = std::make_shared<PriorityCache::Manager>(store->cct, min, max,
													   target, true);
	...
	while (!stop) {
		...
		if (alloc_stats_dump_interval > 0 &&
			alloc_stats_dump_clock + alloc_stats_dump_interval <
				ceph_clock_now()) {
...
void BlueStore::MempoolThread::_resize_shards(bool interval_stats)
{
	...
	uint64_t max_shard_onodes =
		static_cast<uint64_t>((meta_alloc / (double)onode_shards) /
							  meta_cache->get_bytes_per_onode());
	uint64_t max_shard_buffer =
		static_cast<uint64_t>(data_alloc / buffer_shards);
...
int BlueStore::OmapIteratorImpl::seek_to_first()
{
	...
	c->store->log_latency(
		__func__, l_bluestore_omap_seek_to_first_lat,
...
int BlueStore::OmapIteratorImpl::next()
{
	...
	c->store->log_latency(
		__func__, l_bluestore_omap_next_lat, mono_clock::now() - start1,
		c->store->cct->_conf->bluestore_log_omap_iterator_age);
...
void BlueStore::_set_compression()
{
	...
	dout(10)
		<< __func__ << " mode " << Compressor::get_comp_mode_name(comp_mode)
	...
	if (cct->_conf->bluestore_throttle_cost_per_io) {
		...
		if (_use_rotational_settings()) {
			throttle_cost_per_io =
				cct->_conf->bluestore_throttle_cost_per_io_hdd;
		} else {
			throttle_cost_per_io =
				cct->_conf->bluestore_throttle_cost_per_io_ssd;
		}
	}
...
int BlueStore::_set_cache_sizes()
{
	...
	if (cache_meta_ratio + cache_kv_ratio > 1.0) {
		derr << __func__ << " bluestore_cache_meta_ratio (" << cache_meta_ratio
			 ...
			 << ") = " << cache_meta_ratio + cache_kv_ratio
			 << "; must be <= 1.0" << dendl;
		return -EINVAL;
	}
...
{
	PerfCountersBuilder b(cct, "bluestore", l_bluestore_first,
						  l_bluestore_last);
	...
	b.add_u64_counter(l_bluestore_onode_shard_hits,
	...
	b.add_u64_counter(l_bluestore_buffer_miss_bytes,
					  ...
					  "Sum for bytes of read missed in the cache", NULL, 0,
					  unit_t(UNIT_BYTES));
	...
	b.add_u64_counter(
		l_bluestore_write_small_bytes, "bluestore_write_small_bytes",
		"Small writes into existing or sparse small blobs (bytes)", NULL, 0,
		unit_t(UNIT_BYTES));
	...
	b.add_u64_counter(l_bluestore_txc, "bluestore_txc",
	...
	b.add_u64_counter(
		l_bluestore_reads_with_retries, "bluestore_reads_with_retries",
		"Read operations that required at least one retry due to failed checksum validation");
	b.add_u64(
		l_bluestore_fragmentation, "bluestore_fragmentation_micros",
...
int BlueStore::_open_path()
{
	...
	if (path_fd < 0) {
		int r = -errno;
		derr << __func__ << " unable to open " << path << ": "
								 ...
								 bluestore_bdev_label_t label)
{
	...
	if (fd < 0) {
		fd = -errno;
		derr << __func__ << " failed to open " << path << ": "
								...
								bluestore_bdev_label_t *label)
{
	...
	try {
		...
		dout(2) << __func__ << " unable to decode label at offset "
				<< p.get_off() << ": " << e.what() << dendl;
		return -ENOENT;
	}
	if (crc != expected_crc) {
										...
										bool create)
{
	...
	if (create) {
		...
		if (cct->_conf->bluestore_debug_permit_any_bdev_label) {
			dout(20)
				<< __func__ << " bdev " << path << " fsid " << label.osd_uuid
	...
	if (cct->_conf->bluestore_prefer_deferred_size) {
		...
		if (_use_rotational_settings()) {
			prefer_deferred_size =
				cct->_conf->bluestore_prefer_deferred_size_hdd;
		} else {
			prefer_deferred_size =
				cct->_conf->bluestore_prefer_deferred_size_ssd;
		}
	}
...
void BlueStore::_validate_bdev()
{
	...
	if (dev_size < _get_ondisk_reserved() + cct->_conf->bluestore_bluefs_min) {
		dout(1)
			<< __func__ << " main device size " << byte_u_t(dev_size)
...
int BlueStore::_open_fm(KeyValueDB::Transaction t, bool read_only)
{
	...
	if (t) {
		...
		if (cct->_conf->bluestore_debug_prefill > 0) {
			...
			while (!stop && start < end) {
				...
				dout(20) << __func__ << " free 0x" << std::hex << start << "~"
						 << l << " use 0x" << u << std::dec << dendl;
		...
		if (r < 0) {
			derr << __func__
				 << " freelist init failed, error reading bdev label: "
				 << cpp_strerror(r) << dendl;
			delete fm;
			fm = NULL;
			return r;
	...
	if (r < 0) {
		derr << __func__ << " freelist init failed: " << cpp_strerror(r)
			 << dendl;
		delete fm;
		fm = NULL;
		return r;
...
int BlueStore::_write_out_fm_meta(uint64_t target_size, bool update_root_size,
...
int BlueStore::_open_alloc()
{
	...
	if (bluefs) {
		...
		dout(10) << __func__ << " bluefs extents 0x" << std::hex
	...
	dout(1) << __func__ << " loaded " << byte_u_t(bytes) << " in " << num
			<< " extents"
			<< " available " << byte_u_t(alloc->get_free()) << dendl;
...
int BlueStore::_write_fsid()
{
	...
	if (r < 0) {
		r = -errno;
		derr << __func__ << " fsid truncate failed: " << cpp_strerror(r)
			 << dendl;
		return r;
	}
...
int BlueStore::_minimal_open_bluefs(bool create)
{
	...
	if (::stat(bfn.c_str(), &st) == 0) {
		...
		if (bluefs->bdev_support_label(BlueFS::BDEV_DB)) {
			r = _check_or_set_bdev_label(
				bfn, bluefs->get_block_device_size(BlueFS::BDEV_DB),
				"bluefs db", create);
			if (r < 0) {
	...
	if (::stat(bfn.c_str(), &st) == 0) {
		...
		if (bluefs->bdev_support_label(BlueFS::BDEV_WAL)) {
			r = _check_or_set_bdev_label(
				bfn, bluefs->get_block_device_size(BlueFS::BDEV_WAL),
				"bluefs wal", create);
			if (r < 0) {
				derr << __func__ << " check block device(" << bfn
					 << ") label returned: " << cpp_strerror(r) << dendl;
...
int BlueStore::_open_bluefs(bool create)
{
	...
	if (bluefs_layout.shared_bdev == BlueFS::BDEV_SLOW) {
		...
		int r = RocksDBStore::ParseOptionsFromStringStatic(cct, options,
														   rocks_opts, nullptr);
		if (r < 0) {
			return r;
		}
	...
	r = bluefs->mount();
	if (r < 0) {
		derr << __func__ << " failed bluefs mount: " << cpp_strerror(r)
			 << dendl;
	}
...
int BlueStore::_open_db(bool create, bool to_repair_db, bool read_only)
{
	...
	if (do_bluefs) {
		...
		if (cct->_conf->bluestore_bluefs_env_mirror) {
			...
			if (create) {
				string cmd = "rm -rf " + path + "/db " + path + "/db.slow " +
							 path + "/db.wal";
		...
		if (bluefs_layout.shared_bdev == BlueFS::BDEV_SLOW) {
			...
			dout(1)
				<< __func__ << " set db_paths to " << db_paths.str() << dendl;
		}
	...
	if (kv_backend == "rocksdb") {
		...
		cct->_conf.with_val<string>("bluestore_rocksdb_cfs", get_str_map,
									&cf_map, " \t");
		for (auto &i : cf_map) {
			dout(10)
				<< "column family " << i.first << ": " << i.second << dendl;
	...
	if (size) {
		...
		do {
			...
			if (alloc_len < 0 ||
				...
				derr << __func__ << " failed to allocate on 0x" << std::hex
					 << gift << " min_size 0x" << min_size
					 << " > allocated total 0x" << allocated
					 << " bluefs_shared_alloc_size 0x" << alloc_size
					 << " allocated 0x" << (alloc_len < 0 ? 0 : alloc_len)
					 << " available 0x " << alloc->get_free() << std::dec
					 << dendl;
		...
		for (auto &e : *extents) {
			...
			if (!extents_out) {
				bluefs->add_block_extent(bluefs_layout.shared_bdev, e.offset,
										 e.length);
			}
		}
	}
										  ...
										  uint64_t bluefs_total)
{
	...
	dout(10)
		<< __func__ << " bluefs " << byte_u_t(bluefs_free) << " free ("
		<< bluefs_free_ratio << ") bluestore " << byte_u_t(my_free) << " free ("
		<< my_free_ratio << "), bluefs_ratio " << bluefs_ratio << dendl;
	...
	if (bluefs_layout.shared_bdev == BlueFS::BDEV_SLOW) {
		...
		if (p.first != p.second) {
			...
			ss << "spilled over " << byte_u_t(p.second - p.first)
			   << " metadata from 'db' device ("
			   << byte_u_t(db.second - db.first) << " used of "
			   << byte_u_t(db.second) << ") to slow device";
...
int BlueStore::_open_collections()
{
	...
	dout(1) << "[TRACE_RXDB_IN_BS] " << __func__ << ", prefix: " << PREFIX_COLL << ", it_cnt: " << it_cnt << dendl;
#endif
	return 0;
}
...
void BlueStore::_open_statfs()
{
	...
	if (r >= 0) {
		...
		if (size_t(bl.length()) >= sizeof(vstatfs.values)) {
			...
			dout(10)
				<< __func__ << " store_statfs is corrupt, using empty" << dendl;
		}
		_check_legacy_statfs_alert();
	} else {
...
#ifdef TRACE_RXDB_IN_BS
		dout(1) << "[TRACE_RXDB_IN_BS] " << __func__ << ", prefix: " << PREFIX_STAT << ", it_cnt: " << it_cnt << dendl;
	...
	if (size) {
		...
		if (fd >= 0) {
			...
			if (r == 0 && S_ISREG(st.st_mode) && // if it is a regular file
				...
				if (r < 0) {
					r = -errno;
					derr << __func__ << " failed to resize " << name
						 << " file to " << size << ": " << cpp_strerror(r)
						 << dendl;
					VOID_TEMP_FAILURE_RETRY(::close(fd));
					return r;
				}
...
int BlueStore::mkfs()
{
...

	{
		...
		if (r == 0) {
			...
			if (cct->_conf->bluestore_fsck_on_mkfs) {
				...
				if (r > 0) {
					derr << __func__ << " fsck found " << r << " errors"
						 << dendl;
					r = -EIO;
				}
			}
	...
	if (fsid != old_fsid) {
		r = _write_fsid();
		if (r < 0) {
			derr << __func__ << " error writing fsid: " << cpp_strerror(r)
				 << dendl;
			goto out_close_fm;
		}
	}
...
int BlueStore::add_new_bluefs_device(int id, const string &dev_path)
{
	...
	if (id == BlueFS::BDEV_NEWWAL) {
		...
		if (bluefs->bdev_support_label(BlueFS::BDEV_NEWWAL)) {
			r = _check_or_set_bdev_label(
				p, bluefs->get_block_device_size(BlueFS::BDEV_NEWWAL),
				"bluefs wal", true);
			ceph_assert(r == 0);
		}
		...
		if (bluefs->bdev_support_label(BlueFS::BDEV_NEWDB)) {
			r = _check_or_set_bdev_label(
				p, bluefs->get_block_device_size(BlueFS::BDEV_NEWDB),
				"bluefs db", true);
			ceph_assert(r == 0);
		}
	...
	bluefs->add_block_extent(
		id, reserved, bluefs->get_block_device_size(id) - reserved, true);
												 ...
												 int id)
{
	...
	if (id == BlueFS::BDEV_SLOW && target_free < used_space) {
		...
		ceph_assert(r == 0);
		dout(1)
			<< __func__ << " Allocating more space at slow device for BlueFS: +"
	...
	if (r < 0) {
		derr << __func__ << " failed during BlueFS migration, "
			 << cpp_strerror(r) << dendl;
		goto shutdown;
	}
...
void BlueStore::set_cache_shards(unsigned num)
{
	...
	for (unsigned i = oold; i < num; ++i) {
		onode_cache_shards[i] = OnodeCacheShard::create(
			cct, cct->_conf->bluestore_cache_type, logger);
	}
	...
	FSCKDepth depth)
{
	...
	for (auto e : extents) {
		...
		if (depth != FSCK_SHALLOW) {
			...
			if (repairer) {
				repairer->get_space_usage_tracker().set_used(e.offset, e.length,
															 cid, oid);
			}
	...
	if (it) {
		for (it->lower_bound(string()); it->valid(); it->next()) {
			...
			if (key == BLUESTORE_GLOBAL_STATFS_KEY) {
				if (repairer) {
					++errors;
					repairer->remove_key(db, PREFIX_STAT,
			...
			if (get_key_pool_stat(key, &pool_id) < 0) {
				derr << "fsck error: bad key " << key << "in statfs namespece"
					 << dendl;
				if (repairer) {
			...
			try {
				...
				if (repairer) {
					dout(20)
						<< __func__ << " undecodable Pool StatFS record, key:'"
			...
			for (auto &s : o->extent_map.shards) {
				...
				if (s.shard_info->offset >= o->onode.size) {
					derr << "fsck error: " << oid << " shard 0x" << std::hex
						 << s.shard_info->offset << " past EOF at 0x"
	...
	for (auto &i : ref_map) {
		...
		if (blob.is_shared()) {
			...
			errors += _fsck_check_extents(c->cid, oid, blob.get_extents(),
										  blob.is_compressed(), *used_blocks,
										  fm->get_alloc_size(), repairer,
										  *res_statfs, depth);
		} else {
			errors += _fsck_sum_extents(blob.get_extents(),
										blob.is_compressed(), *res_statfs);
		}
	} // for (auto& i : ref_map)
     ...
     * and must therefore be thread-safe. */
		void _void_process(void *item, TPHandle &handle) override
		{
			...
			BlueStore::FSCK_ObjectCtx ctx(
				...
				batch->num_extents, batch->num_blobs,
				batch->num_sharded_objects, batch->num_spanning_blobs,
				nullptr, // used_blocks
										...
										const BlueStore::FSCK_ObjectCtx &ctx)
{
	...
	if (repairer && !o->onode.is_perpool_omap() && !o->onode.is_pgmeta_omap()) {
		dout(10)
			<< "fsck converting " << o->oid << " omap to per-pool" << dendl;
		bufferlist h;
									...
									BlueStore::FSCK_ObjectCtx &ctx)
{
	...
	if (it) {
		const size_t thread_count =
			cct->_conf->bluestore_fsck_quick_fix_threads;
		...
		for (it->lower_bound(string()); it->valid(); it->next()) {
			...
			if (!c || oid.shard_id != pgid.shard ||
				...
				dout(20) << __func__ << "  collection " << c->cid << " "
						 << c->cnode << dendl;
			}
			...
			if (!queued) {
				...
				o = fsck_check_objects_shallow(depth, pool_id, c, oid,
											   it->key(), it->value(),
			...
			if (depth != FSCK_SHALLOW) {
				...
				for (auto &i : referenced) {
					dout(20)
						<< __func__ << "  referenced 0x" << std::hex << i.second
						<< std::dec << " for " << *i.first << dendl;
					const bluestore_blob_t &blob = i.first->get_blob();
					if (i.second & blob.unused) {
						derr << "fsck error: " << oid
							 << " blob claims unused 0x" << std::hex
							 << blob.unused << " but extents reference 0x"
							 << i.second << std::dec << " on blob " << *i.first
							 << dendl;
						++errors;
					}
					if (blob.has_csum()) {
						...
						for (unsigned p = 0; p < csum_count; ++p) {
							...
							if ((blob.unused & mask) == mask) {
								...
								if (blob.get_csum_item(p) != 0) {
									derr << "fsck error: " << oid
										 << " blob claims csum chunk 0x"
										 << std::hex << pos << "~"
										 << csum_chunk_size
										 << " is unused (mask 0x" << mask
										 << " of unused 0x" << blob.unused
										 << ") but csum is non-zero 0x"
										 << blob.get_csum_item(p) << std::dec
				...
				if (o->onode.has_omap()) {
					...
					if (ctx.used_omap_head->count(o->onode.nid)) {
						derr << "fsck error: " << o->oid << " omap_head "
							 << o->onode.nid << " already in use" << dendl;
						++errors;
					} else {
				...
				if (depth == FSCK_DEEP) {
					bufferlist bl;
					uint64_t max_read_block =
						cct->_conf->bluestore_fsck_read_bytes_cap;
					uint64_t offset = 0;
					do {
						uint64_t l = std::min(uint64_t(o->onode.size - offset),
											  max_read_block);
		...
		if (depth == FSCK_SHALLOW && thread_count > 0) {
			...
			if (processed_myself) {
				...
				dout(0) << __func__ << " partial offload"
						...
						<< ctx.num_objects << "objects, threads "
						<< thread_count << dendl;
			}
		}
	} // if (it)
#ifdef TRACE_RXDB_IN_BS
	dout(1) << "[TRACE_RXDB_IN_BS] " << __func__ << ", prefix: " << PREFIX_OBJ << ", it_cnt: " << it_cnt << dendl;
#endif
}
/**
...
int BlueStore::_fsck_on_open(BlueStore::FSCKDepth depth, bool repair)
{
	...
	if (repair) {
		repairer.get_space_usage_tracker().init(bdev->get_size(),
												min_alloc_size);
	}
	...
	// walk PREFIX_OBJ
	{
		...
		BlueStore::FSCK_ObjectCtx ctx(
			...
			num_sharded_objects, num_spanning_blobs, &used_blocks,
			&used_omap_head,
		...
		for (it->lower_bound(string()); it->valid(); it->next()) {
			...
			if (p == sb_info.end()) {
				...
				try {
					...
					if (repair) {
						dout(20)
							<< __func__ << " undecodable Shared Blob, key:'"
				...
				dout(20) << __func__ << "  " << *sbi.sb << " " << shared_blob
						 << dendl;
				...
				for (auto &r : shared_blob.ref_map.ref_map) {
					extents.emplace_back(
						bluestore_pextent_t(r.first, r.second.length));
				}
				if (per_pool_stat_collection || repair) {
					expected_statfs = &expected_pool_statfs[sbi.pool_id];
				}
				errors += _fsck_check_extents(sbi.cid, p->second.oids.front(),
											  extents, p->second.compressed,
											  used_blocks, fm->get_alloc_size(),
											  repair ? &repairer : nullptr,
...
#ifdef TRACE_RXDB_IN_BS
	dout(1) << "[TRACE_RXDB_IN_BS] " << __func__ << ", prefix: " << PREFIX_SHARED_BLOB << ", it_cnt: " << it_cnt << dendl;
#endif
		...
		if (it) {
			...
			for (it->lower_bound(string()); it->valid() && !bypass_rest;
				...
				if (!c || oid.shard_id != pgid.shard ||
					...
					if (per_pool_stat_collection || repair) {
						auto pool_id = c->cid.is_pg(&pgid) ? pgid.pool() :
															   META_POOL_ID;
				...
				for (auto b : blobs) {
					...
					for (auto e = pextents.begin(); e != pextents.end();
						...
						PExtentVector exts;
						int64_t alloc_len = alloc->allocate(
							e->length, min_alloc_size, 0, 0, &exts);
						if (alloc_len < 0 || alloc_len < (int64_t)e->length) {
							derr << __func__ << " failed to allocate 0x"
								 << std::hex << e->length << " allocated 0x "
								 << (alloc_len < 0 ? 0 : alloc_len)
						...
						if (r < 0) {
							derr << __func__ << " failed to read from 0x"
								 << std::hex << e->offset << "~" << e->length
								 << std::dec << dendl;
			...
			for (auto it = to_release.begin(); it != to_release.end(); ++it) {
				dout(10)
					<< __func__ << " release 0x" << std::hex << it.get_start()
			...
			if (repair && (!sbi.passed || sbi.updated)) {
				...
				if (sbi.ref_map.empty()) {
					...
					bluestore_shared_blob_t persistent(sbid,
													   std::move(sbi.ref_map));
					encode(persistent, bl);
					dout(20) << __func__ << " " << *sbi.sb << " is "
							 << bl.length() << " bytes, updating" << dendl;
	...
	if (depth != FSCK_SHALLOW) {
...
#ifdef TRACE_RXDB_IN_BS
		dout(1) << "[TRACE_RXDB_IN_BS] " << __func__ << ", prefix: " << PREFIX_OMAP << ", it_cnt: " << it_cnt << dendl;
#endif
...
#ifdef TRACE_RXDB_IN_BS
		dout(1) << "[TRACE_RXDB_IN_BS] " << __func__ << ", prefix: " << PREFIX_PGMETA_OMAP << ", it_cnt: " << it_cnt << dendl;
#endif
...
#ifdef TRACE_RXDB_IN_BS
		dout(1) << "[TRACE_RXDB_IN_BS] " << __func__ << ", prefix: " << PREFIX_PERPOOL_OMAP << ", it_cnt: " << it_cnt << dendl;
#endif
		}
		...
		if (it) {
			for (it->lower_bound(string()); it->valid(); it->next()) {
				...
				dout(20) << __func__ << "  deferred " << wt.seq << " ops "
						 << wt.ops.size() << " released 0x" << std::hex
						 << wt.released << std::dec << dendl;
				for (auto e = wt.released.begin(); e != wt.released.end();
					 ++e) {
					apply_for_bitset_range(e.get_start(), e.get_len(),
										   fm->get_alloc_size(), used_blocks,
										   [&](uint64_t pos,
											   mempool_dynamic_bitset &bs) {
											   bs.set(pos);
										   });
				}
			}
...
#ifdef TRACE_RXDB_IN_BS
		dout(1) << "[TRACE_RXDB_IN_BS] " << __func__ << ", prefix: " << PREFIX_DEFERRED << ", it_cnt: " << it_cnt << dendl;
#endif
		...
		dout(1) << __func__ << " checking freelist vs allocated" << dendl;
		{
			...
			for (auto e = bluefs_extents.begin(); e != bluefs_extents.end();
				 ++e) {
				apply_for_bitset_range(
					e.get_start(), e.get_len(), fm->get_alloc_size(),
					used_blocks, [&](uint64_t pos, mempool_dynamic_bitset &bs) {
						bs.reset(pos);
					});
			}
			fm->enumerate_reset();
	...
	for (auto pext : v) {
		if (pext->size()) {
			...
			while (p != pext->end()) {
				if (p->is_valid()) {
					dout(20)
						<< __func__ << " release 0x" << std::hex << p->offset
...
void BlueStore::collect_metadata(map<string, string> *pm)
{
	...
	if (r >= 0) {
		...
		if (!nodes.empty()) {
			dout(1)
				<< __func__ << " devices span numa nodes " << nodes << dendl;
							 ...
							 set<string> *out_failed)
{
	...
	for (auto &devname : devices) {
		...
		if (r < 0) {
			dout(10) << __func__ << " bdev " << devname
					 << " can't detect numa_node" << dendl;
	...
	while (p != removed_colls.end()) {
		...
		if (c->onode_map.map_any([&](Onode *o) {
				...
				if (o->flushing_count.load()) {
					dout(10) << __func__ << " " << c << " " << c->cid << " "
							 << o->oid << " flush_txns " << o->flushing_count
							 << dendl;
					return true;
				}
				return false;
					...
					struct stat *st, bool allow_eio)
{
	...
	if (_debug_mdata_eio(oid)) {
		r = -EIO;
		derr << __func__ << " " << c->cid << " " << oid << " INJECT EIO"
			 << dendl;
	}
	return r;
}
							...
							blobs2read_t &blobs2read)
{
	...
	while (left > 0 && lp != o->extent_map.extent_map.end()) {
		if (pos < lp->logical_offset) {
			...
			dout(30) << __func__ << "  hole 0x" << std::hex << pos << "~"
					 << hole << std::dec << dendl;
			pos += hole;
			left -= hole;
		}
		...
		while (b_len > 0) {
			...
			if (pc != cache_res.end() && pc->first == b_off) {
				...
				dout(30) << __func__ << "    use cache 0x" << std::hex << pos
				...
				dout(30) << __func__ << "    will read 0x" << std::hex << pos
						 << ": 0x" << b_off << "~" << l << std::dec << dendl;
				// merge regions
				{
					...
					if (r2r.size()) {
						...
						if (r_off <= (pre.r_off + pre.r_len)) {
							...
							pre.r_len +=
								(r_off + r_len - pre.r_off - pre.r_len);
							pre.regs.emplace_back(
								region_t(pos, b_off, l, front));
							merged = true;
						}
					}
					if (!merged) {
								 ...
								 IOContext *ioc)
{
	for (auto &p : blobs2read) {
		...
		dout(20) << __func__ << "  blob " << *bptr << std::hex << " need "
				 << r2r << std::dec << dendl;
		if (bptr->get_blob().is_compressed()) {
			...
			for (auto &req : r2r) {
				dout(20) << __func__ << "    region 0x" << std::hex
						 ...
						 << req.regs.front().blob_xoffset << " reading 0x"
						 << req.r_off << "~" << req.r_len << std::dec << dendl;
	...
	while (b2r_it != blobs2read.end()) {
		...
		if (bptr->get_blob().is_compressed()) {
			...
			for (auto &req : r2r) {
				...
				if (buffered) {
					bptr->shared_blob->bc.did_read(
						bptr->shared_blob->get_cache(), req.r_off, req.bl);
				}

				// prune and keep result
				for (const auto &r : req.regs) {
					ready_regions[r.logical_offset].substr_of(req.bl, r.front,
															  r.length);
				}
			}
		}
						...
						uint64_t retry_count)
{
	...
	if (op_flags & CEPH_OSD_OP_FLAG_BYPASS_CLEAN_CACHE) {
		dout(20)
			<< __func__ << " will bypass cache and do direct read" << dendl;
	...
	ready_regions_t ready_regions;
	blobs2read_t blobs2read;
	_read_cache(o, offset, length, read_cache_policy, ready_regions,
				blobs2read);
	...
	if (retry_count) {
		...
		dout(5) << __func__ << " read at 0x" << std::hex << offset << "~"
				<< length << " failed " << std::dec << retry_count
	...
	if (r < 0) {
		if (r == -1) {
			...
			derr << __func__ << " bad "
				 ...
				 << " checksum at blob offset 0x" << bad << ", got 0x"
				 << bad_csum << ", expected 0x"
				 ...
				 << blob->get_csum_chunk_size() << std::dec << ", object "
				 << o->oid << dendl;
		} else {
		...
		return -ENOENT;
	{
		...
		while (length > 0) {
			...
			if (ep != eend && ep->logical_offset <= offset) {
				...
				dout(30)
					<< __func__ << " lextent 0x" << std::hex << offset << "~"
					 ...
					 uint32_t op_flags)
{
	...
	const coll_t &cid = c->get_cid();
	dout(15)
		<< __func__ << " " << cid << " " << oid << " fiemap " << m << dendl;
	if (!c->exists)
		return -ENOENT;
						 ...
						 uint32_t op_flags, uint64_t retry_count)
{
	...
	dout(20)
		<< __func__ << " fiemap " << m << std::hex << " size 0x"
	...
	for (auto p = m.begin(); p != m.end(); p++, i++) {
		...
		r = _generate_read_result_bl(o, p.get_start(), p.get_len(),
									 ...
									 std::get<2>(raw_results[i]), buffered,
									 &csum_error, t);
		if (csum_error) {
			// Handles spurious read errors caused by a kernel bug.
			// We sometimes get all-zero pages as a result of the read under
							   ...
							   vector<ghobject_t> *ls, ghobject_t *pnext)
{
	Collection *c = static_cast<Collection *>(c_.get());
	c->flush();
	dout(15) << __func__ << " " << c->cid << " start " << start << " end "
			 << end << " max " << max << dendl;
	int r;
	{
		std::shared_lock l(c->lock);
	...
	dout(10)
		<< __func__ << " " << c->cid << " start " << start << " end " << end
								...
								vector<ghobject_t> *ls, ghobject_t *pnext)
{
	...
	if (start == ghobject_t() || start.hobj == hobject_t() ||
		...
		if (start.hobj.is_temp()) {
			temp = true;
			ceph_assert(start >= coll_range_temp_start &&
						start < coll_range_temp_end);
		} else {
			temp = false;
	...
	while (true) {
		if (!it->valid() || it->is_ge(pend)) {
			...
			if (!it->valid())
				...
				dout(20) << __func__ << " oid " << it->oid() << " >= " << pend
						 << dendl;
			if (temp) {
				if (end.hobj.is_temp()) {
				...
				dout(30)
					<< __func__ << " switch to non-temp namespace" << dendl;
				temp = false;
		...
		while (it->valid()) {
			...
			if (it->key() == head) {
				...
				*header = it->value();
				...
				dout(20)
					<< __func__ << "  got " << pretty_binary_string(it->key())
					<< " -> " << user_key << dendl;
...
#ifdef TRACE_RXDB_IN_BS
		dout(1) << "[TRACE_RXDB_IN_BS] " << __func__ << ", prefix: " << prefix << ", it_cnt: " << it_cnt << dendl;
#endif
	}
out:
	return r;
...
out:
	c->store->log_latency(
		__func__, l_bluestore_omap_get_keys_lat, mono_clock::now() - start1,
		c->store->cct->_conf->bluestore_log_omap_iterator_age);
...
)
{
	...
	o->flush();
	{
		...
		o->get_omap_key(string(), &final_key);
		size_t base_key_len = final_key.size();
		for (set<string>::const_iterator p = keys.begin(); p != keys.end();
			 ++p) {
			...
			if (db->get(prefix, final_key, &val) >= 0) {
				dout(30)
					<< __func__ << "  got " << pretty_binary_string(final_key)
					<< " -> " << *p << dendl;
...
#ifdef TRACE_RXDB_IN_BS
		dout(1) << "[TRACE_RXDB_IN_BS] " << __func__ << ", prefix(oid)" << ", it_cnt: " << it_cnt << dendl;
#endif
	}
...
#ifdef TRACE_RXDB_IN_BS
	dout(1) << "[TRACE_RXDB_IN_BS] " << __func__ <<", it_cnt: " << it_cnt << dendl;
#endif
	return r;
}
#endif
...
)
{
	...
	o->flush();
	{
		...
		for (set<string>::const_iterator p = keys.begin(); p != keys.end();
			...
			if (db->get(prefix, final_key, &val) >= 0) {
				dout(30)
					<< __func__ << "  have " << pretty_binary_string(final_key)
					<< " -> " << *p << dendl;
				out->insert(*p);
			} else {
				dout(30)
					<< __func__ << "  miss " << pretty_binary_string(final_key)
					<< " -> " << *p << dendl;
			}
		}
	}
...
{
	dout(10)
		<< __func__ << " ondisk_format " << ondisk_format
...
int BlueStore::_open_super_meta()
{
		...
		if (r < 0) {
			...
			bl.clear();
			{
				...
				try {
					...
					derr << __func__ << " unable to read compat_ondisk_format"
						 << dendl;
					return -EIO;
				}
			}
	...
	if (latest_ondisk_format < compat_ondisk_format) {
		derr << __func__ << " compat_ondisk_format is " << compat_ondisk_format
			 << " but we only understand version " << latest_ondisk_format
			 << dendl;
		return -EPERM;
	}

	{
		...
		dout(1) << __func__ << " min_alloc_size 0x" << std::hex
		...
		if (ondisk_format == 1) {
			...
			// - super: removed min_min_alloc_size
			{
				...
				try {
					...
					derr << __func__ << " failed to read min_min_alloc_size"
						 << dendl;
					return -EIO;
				}
				t->set(PREFIX_SUPER, "min_alloc_size", bl);
												...
												list<Context *> *on_commits)
{
	...
	osr->queue_new(txc);
	dout(20) << __func__ << " osr " << osr << " = " << txc << " seq "
			 << txc->seq << dendl;
	return txc;
}
...
void BlueStore::_txc_write_nodes(TransContext *txc, KeyValueDB::Transaction t)
{
	...
	for (auto sb : txc->shared_blobs) {
		...
		if (sb->persistent->empty()) {
			dout(20) << __func__ << " shared_blob 0x" << std::hex << sbid
					 << std::dec << " is empty" << dendl;
			t->rmkey(PREFIX_SHARED_BLOB, key);
		} else {
			bufferlist bl;
			encode(*(sb->persistent), bl);
			dout(20)
				<< __func__ << " shared_blob 0x" << std::hex << sbid << std::dec
...
void BlueStore::_txc_finish(TransContext *txc)
{
	...
	if (empty && osr->zombie) {
		...
		if (zombie_osr_set.erase(osr->cid)) {
			dout(10)
				<< __func__ << " reaping empty zombie osr " << osr << dendl;
		} else {
...
void BlueStore::_txc_release_alloc(TransContext *txc)
{
	...
	if (likely(!cct->_conf->bluestore_debug_no_reuse_blocks)) {
		...
		if (cct->_conf->bdev_enable_discard && cct->_conf->bdev_async_discard) {
			...
			for (auto p = txc->released.begin(); p != txc->released.end();
				 ++p) {
...
void BlueStore::_osr_attach(Collection *c)
{
	...
	if (q != coll_map.end()) {
		...
		if (p == zombie_osr_set.end()) {
			c->osr =
				ceph::make_ref<OpSequencer>(this, next_sequencer_id++, c->cid);
			ldout(cct, 10) << __func__ << " " << c->cid << " fresh osr "
						   << c->osr << dendl;
		} else {
			c->osr = p->second;
...
void BlueStore::_kv_sync_thread()
{
	...
	while (true) {
		...
		if (period && elapsed >= observation_period) {
			dout(5) << __func__ << " utilization: idle " << twait << " of "
					<< elapsed << ", submitted: " << kv_submitted << dendl;
			t0 = mono_clock::now();
		...
		if (kv_queue.empty() &&
			...
			dout(20) << __func__ << " committing " << kv_queue.size()
					 ...
					 << " stable " << deferred_stable_queue.size() << dendl;
			...
			dout(30)
				<< __func__ << " deferred_stable " << deferred_stable << dendl;
			...
			if (bluefs && bluefs_layout.single_shared_device()) {
				...
				if (aios || !deferred_done.empty()) {
					...
					dout(20) << __func__
							 << " skipping flush (no aios, no deferred_done)"
							 << dendl;
				}
			}
			...
			if (blobid_last + cct->_conf->bluestore_blobid_prealloc / 2 >
				...
				KeyValueDB::Transaction t =
					kv_submitting.empty() ? synct : kv_submitting.front()->t;
				new_blobid_max =
					blobid_last + cct->_conf->bluestore_blobid_prealloc;
				bufferlist bl;
				encode(new_blobid_max, bl);
				t->set(PREFIX_SUPER, "blobid_max", bl);
				dout(10) << __func__ << " new_blobid_max " << new_blobid_max
						 << dendl;
			}
...

			{
				...
				if (deferred_stable_to_finalize.empty()) {
					...
					deferred_stable_to_finalize.insert(
						...
						deferred_stable.begin(), deferred_stable.end());
			...
			if (new_blobid_max) {
				...
				dout(10)
					<< __func__ << " blobid_max now " << blobid_max << dendl;
			}

			{
				...
				dout(20) << __func__ << " committed " << committing_size
						 << " cleaned " << deferred_size << " in " << dur
						 << " (" << dur_flush << " flush + " << dur_kv
						 << " kv commit)" << dendl;
			...
			if (bluefs) {
				if (!bluefs_extents_reclaiming.empty()) {
					dout(0)
						<< __func__ << " releasing old bluefs 0x" << std::hex
...
void BlueStore::_deferred_queue(TransContext *txc)
{
	...
	for (auto opi = wt.ops.begin(); opi != wt.ops.end(); ++opi) {
		...
		for (auto e : op.extents) {
			txc->osr->deferred_pending->prepare_write(cct, wt.seq, e.offset,
													  e.length, p);
		}
	}
		...
		if (osr->deferred_pending) {
			if (!osr->deferred_running) {
				...
				dout(20) << __func__ << "  osr " << osr
						 << " already has running" << dendl;
			}
		} else {
			dout(20)
				<< __func__ << "  osr " << osr << " has no pending" << dendl;
		}
	}
...
void BlueStore::_deferred_aio_finish(OpSequencer *osr)
{
...

	{
		...
		if (!osr->deferred_pending) {
			...
			deferred_lock.unlock();
			if (deferred_aggressive) {
				dout(20) << __func__ << " queuing async deferred_try_submit"
						 << dendl;
				finisher.queue(new C_DeferredTrySubmit(this));
			} else {
				dout(20)
					<< __func__ << " leaving queued, more pending" << dendl;
			}
		}
	}
								  ...
								  ThreadPool::TPHandle *handle)
{
	...
	if (!throttle.try_start_transaction(*db, *txc, tstart)) {
		...
		dout(10) << __func__
				 << " failed get throttle_deferred_bytes, aggressive" << dendl;
		++deferred_aggressive;
		deferred_try_submit();
		{
			// wake up any previously finished deferred events
	...
	for (int pos = 0; i.have_op(); ++pos) {
		...
		case Transaction::OP_COLL_HINT: {
			...
			if (type == Transaction::COLL_HINT_EXPECTED_NUM_OBJECTS) {
				...
				// Ignore the hint
				dout(10)
					<< __func__ << " unknown collection hint " << type << dendl;
			}
			continue;
		} break;
								...
								bufferlist::iterator &blp, WriteContext *wctx)
{
	...
	do {
		...
		if (ep != end && ep->logical_offset < offset + max_bsize) {
			...
			dout(20) << __func__ << " considering " << *b << " bstart 0x"
					 << std::hex << bstart << std::dec << dendl;
			if (bstart >= end_offs) {
				...
				dout(20)
					<< __func__ << " ignoring offset-skewed " << *b << dendl;
			} else {
				...
				if (head_pad || tail_pad) {
					o->extent_map.fault_range(db, offset - head_pad,
											  end_offs - offset + head_pad +
												  tail_pad);
				}
				if (head_pad && o->extent_map.has_any_lextents(
									offset - head_pad, chunk_size)) {
					head_pad = 0;
				}
				if (tail_pad &&
				...
				if ((b_off % chunk_size == 0 && b_len % chunk_size == 0) &&
					...
					if (!g_conf()->bluestore_debug_omit_block_device_write) {
						if (b_len <= prefer_deferred_size) {
							dout(20) << __func__ << " deferring small 0x"
									 << std::hex << b_len << std::dec
				...
				if (b->get_blob().get_ondisk_length() >= b_off + b_len &&
					...
					dout(20) << __func__ << "  reading head 0x" << std::hex
							 << head_read << " and tail 0x" << tail_read
							 << std::dec << dendl;
					if (head_read) {
						bufferlist head_bl;
						int r = _do_read(c.get(), o,
										 offset - head_pad - head_read,
										 head_read, head_bl, 0);
					...
					if (tail_read) {
						bufferlist tail_bl;
						int r = _do_read(c.get(), o, offset + length + tail_pad,
										 tail_read, tail_bl, 0);
					...
					if (!g_conf()->bluestore_debug_omit_block_device_write) {
						...
						ceph_assert(r == 0);
						op->data.claim(bl);
						dout(20) << __func__ << "  deferred write 0x"
								 << std::hex << b_off << "~" << b_len
								 << std::dec << " of mutable " << *b << " at "
								 << op->extents << dendl;
					}
				...
				if (b->can_reuse_blob(min_alloc_size, max_bsize,
									  offset0 - bstart, &alloc_len)) {
					ceph_assert(alloc_len ==
								min_alloc_size); // expecting data always
					...
					if (!wctx->has_conflict(b, offset0, offset0 + alloc_len,
						...
						dout(20)
							<< __func__ << " reuse blob " << *b << std::hex
							<< " (0x" << b_off0 << "~" << bl.length() << ")"
							<< " (0x" << b_off << "~" << length << ")"
							<< std::dec << dendl;

						o->extent_map.punch_hole(c, offset, length,
												 &wctx->old_extents);
		...
		if (prev_ep != end && prev_ep->logical_offset >= min_off) {
			...
			if (b->can_reuse_blob(min_alloc_size, max_bsize, offset0 - bstart,
				...
				if (!wctx->has_conflict(b, offset0, offset0 + alloc_len,
										min_alloc_size)) {
					uint64_t chunk_size =
						b->get_blob().get_chunk_size(block_size);
					uint64_t b_off = offset - bstart;
					uint64_t b_off0 = b_off;
					_pad_zeros(&bl, &b_off0, chunk_size);
	...
	if (above_blob_threshold) {
		...
		dout(20) << __func__ << " inserting (last) for GC " << std::hex
				 << offset << "~" << length << std::dec << dendl;
	}
	// new blob.
	BlobRef b = c->new_blob();
							   ...
							   OnodeRef o, WriteContext *wctx)
{
				...
				if (!cp || cp->get_type_name() != val) {
					...
					if (!cp) {
						if (_set_compression_alert(false, val.c_str())) {
							derr << __func__ << " unable to initialize "
								 << val.c_str() << " compressor" << dendl;
						}
					}
				}
	...
	for (auto &wi : wctx->writes) {
		if (c && wi.blob_length > min_alloc_size) {
			...
			if (r == 0 && result_len <= want_len &&
				result_len < wi.blob_length) {
				bluestore_compression_header_t chdr;
				...
				if (result_len <= want_len && result_len < wi.blob_length) {
					...
					wi.compressed = true;
					logger->inc(l_bluestore_write_pad_bytes,
								result_len - compressed_len);
					dout(20) << __func__ << std::hex << "  compressed 0x"
							 << wi.blob_length << " -> 0x" << compressed_len
							 << " => 0x" << result_len << " with "
							 << c->get_type() << std::dec << dendl;
			...
			if (rejected) {
				dout(20) << __func__ << std::hex << "  0x" << wi.blob_length
						 ...
						 << " -> 0x" << want_len << ", leaving uncompressed"
						 << std::dec << dendl;
				logger->inc(l_bluestore_compress_rejected_count);
				need += wi.blob_length;
			}
		...
		if (wi.compressed) {
			...
			if (csum != Checksummer::CSUM_NONE) {
				dout(20)
					...
					<< std::hex << csum_length << " blob_length 0x"
					<< wi.blob_length << " compressed_length 0x"
			...
			if (l->length() != wi.blob_length) {
				...
				dout(20)
					<< __func__ << " forcing csum_order to block_size_order "
			...
			if (csum != Checksummer::CSUM_NONE) {
				dout(20) << __func__ << " initialize csum setting for new blob "
						 << *b << " csum_type "
						 << Checksummer::get_csum_type_string(csum)
						 << " csum_order " << csum_order << " csum_length 0x"
		...
		if (!g_conf()->bluestore_debug_omit_block_device_write) {
			if (l->length() <= prefer_deferred_size.load()) {
				dout(20) << __func__ << " deferring small 0x" << std::hex
						 << l->length() << std::dec << " write via deferred"
						 << dendl;
				bluestore_deferred_op_t *op = _get_deferred_op(txc);
				op->op = bluestore_deferred_op_t::OP_WRITE;
				int r = b->get_blob().map(
				...
				b->get_blob().map_bl(
					b_off, *l, [&](uint64_t offset, bufferlist &t) {
	...
	while (oep != wctx->old_extents.end()) {
		...
		if (!r.empty()) {
			...
			if (blob.is_shared()) {
				...
				bool unshare = false;
				bool *unshare_ptr =
					!maybe_unshared_blobs || b->is_referenced() ? nullptr :
																	&unshare;
				for (auto e : r) {
									  ...
									  WriteContext *wctx)
{
	...
	if ((alloc_hints & CEPH_OSD_ALLOC_HINT_FLAG_SEQUENTIAL_READ) &&
		...
		dout(20)
			<< __func__ << " will prefer large blob and csum sizes" << dendl;
						...
						uint64_t offset, size_t length)
{
	...
	if (length > 0 && offset + length > o->onode.size) {
		...
		dout(20)
			<< __func__ << " extending size to " << offset + length << dendl;
	}
	txc->write_onode(o);
...
int BlueStore::_do_remove(TransContext *txc, CollectionRef &c, OnodeRef o)
{
	...
	for (auto &s : o->extent_map.shards) {
		...
		generate_extent_shard_key_and_apply(
			o->key, s.shard_info->offset, &key, [&](const string &final_key) {
...
{
	dout(15) << __func__ << " " << c->cid << " " << o->oid << " onode "
			 << o.get() << " txc " << txc << dendl;

	auto start_time = mono_clock::now();
	int r = _do_remove(txc, c, o);
	log_latency_fn(__func__, l_bluestore_remove_lat,
				   mono_clock::now() - start_time,
				   cct->_conf->bluestore_log_op_age,
				   [&](const ceph::timespan &lat) {
					   ostringstream ostr;
...
{
	dout(15)
		<< __func__ << " " << c->cid << " " << o->oid << " " << name << dendl;
	int r = 0;
...
void BlueStore::_do_omap_clear(TransContext *txc, OnodeRef &o)
{
	...
	dout(20)
		<< __func__ << " remove range start: " << pretty_binary_string(prefix)
...
{
	dout(15)
		<< __func__ << " " << c->cid << " " << oldo->oid << " -> " << newo->oid
					   ...
					   OnodeRef &newo, const ghobject_t &new_oid)
{
	...
	// rewrite shards
	{
		...
		for (auto &s : oldo->extent_map.shards) {
			generate_extent_shard_key_and_apply(
				oldo->key, s.shard_info->offset, &key,
				[&](const string &final_key) {
					txc->t->rmkey(PREFIX_OBJ, final_key);
				});
			s.dirty = true;
...
out:
	dout(10)
		<< __func__ << " " << cid << " bits " << bits << " = " << r << dendl;
	return r;
}
	...
	mono_clock::time_point start_throttle_acquire)
{
	...
	if (should_trace(&started, &completed)) {
		...
		db.get_property("rocksdb.compaction-pending",
		...
		tracepoint(bluestore, transaction_initial_state_rocksdb,
				   ...
				   rocksdb_cur_size_all_mem_tables, rocksdb_compaction_pending,
				   rocksdb_mem_table_flush_pending,
				   rocksdb_num_running_compactions, rocksdb_num_running_flushes,
				   rocksdb_actual_delayed_write_rate);
	}
}
#endif
...
mono_clock::duration
BlueStore::BlueStoreThrottle::log_state_latency(TransContext &txc,
												PerfCounters *logger, int state)
{
	...
	if (txc.tracing && state >= l_bluestore_state_prepare_lat &&
		...
		OID_ELAPSED("", lat.to_nsec() / 1000.0,
					txc.get_state_latency_name(state));
...
void BlueStore::generate_db_histogram(Formatter *f)
{
	...
	while (iter->valid()) {
		...
		if (key.first == PREFIX_SUPER) {
			hist.update_hist_entry(hist.key_hist, PREFIX_SUPER, key_size,
								   value_size);
			num_super++;
		} else if (key.first == PREFIX_STAT) {
			hist.update_hist_entry(hist.key_hist, PREFIX_STAT, key_size,
								   value_size);
			num_stat++;
		} else if (key.first == PREFIX_COLL) {
			hist.update_hist_entry(hist.key_hist, PREFIX_COLL, key_size,
								   value_size);
			num_coll++;
			...
			if (key.second.back() == ONODE_KEY_SUFFIX) {
				...
				hist.update_hist_entry(hist.key_hist, prefix_onode_shard,
									   key_size, value_size);
				num_shards++;
			}
		} else if (key.first == PREFIX_OMAP) {
			hist.update_hist_entry(hist.key_hist, PREFIX_OMAP, key_size,
								   value_size);
			num_omap++;
			...
			hist.update_hist_entry(hist.key_hist, PREFIX_DEFERRED, key_size,
								   value_size);
			num_deferred++;
		} else if (key.first == PREFIX_ALLOC ||
				   key.first == PREFIX_ALLOC_BITMAP) {
			hist.update_hist_entry(hist.key_hist, PREFIX_ALLOC, key_size,
								   value_size);
			num_alloc++;
			...
			hist.update_hist_entry(hist.key_hist, prefix_other, key_size,
								   value_size);
			num_others++;
		}
		iter->next();
...
void BlueStore::_record_onode(OnodeRef &o, KeyValueDB::Transaction &txn)
{
	...
	if (o->extent_map.needs_reshard()) {
		...
		if (o->extent_map.needs_reshard()) {
			dout(20)
				<< __func__ << " warning: still wants reshard, check options?"
				<< dendl;
...
void BlueStore::_log_alerts(osd_alert_list_t &alerts)
{
	...
	if (!disk_size_mismatch_alert.empty()) {
		alerts.emplace("BLUESTORE_DISK_SIZE_MISMATCH",
...
uint8_t RocksDBBlueFSVolumeSelector::select_prefer_bdev(void *h)
{
	...
	case LEVEL_SLOW:
		...
		if (db_avail4slow > 0) {
			...
			max_db_use += per_level_per_dev_max.at(BlueFS::BDEV_DB,
												   LEVEL_LOG - LEVEL_FIRST);
			max_db_use += per_level_per_dev_max.at(BlueFS::BDEV_DB,
												   LEVEL_WAL - LEVEL_FIRST);
			max_db_use += per_level_per_dev_max.at(BlueFS::BDEV_DB,
												   LEVEL_DB - LEVEL_FIRST);
			// this could go to db hence using it in the estimation
			max_db_use += per_level_per_dev_max.at(BlueFS::BDEV_SLOW,
												   LEVEL_DB - LEVEL_FIRST);

			auto db_total = l_totals[LEVEL_DB - LEVEL_FIRST];
			uint64_t avail = min(db_avail4slow, max_db_use < db_total ?
													db_total - max_db_use :
													  0);
