...
public:
			   ...
			   rocksdb::Logger *logger) const override
	{
		...
		for (auto &p : store.merge_ops) {
			if (p.first.compare(0, p.first.length(), key.data(),
								p.first.length()) == 0 &&
				key.data()[p.first.length()] == 0) {
				if (existing_value) {
					p.second->merge(existing_value->data(),
									existing_value->size(), value.data(),
									value.size(), new_value);
				} else {
					p.second->merge_nonexistent(value.data(), value.size(),
												new_value);
				}
				break;
			}
...
int RocksDBStore::set_merge_operator(
	const string &prefix, std::shared_ptr<KeyValueDB::MergeOperator> mop)
{
...
int RocksDBStore::tryInterpret(const string &key, const string &val,
							   rocksdb::Options &opt)
{
...
int RocksDBStore::ParseOptionsFromStringStatic(
	CephContext *cct, const string &opt_str, rocksdb::Options &opt,
	function<int(const string &, const string &, rocksdb::Options &)> interp)
{
	// keep aligned with func tryInterpret
	const set<string> need_interp_keys = { "compaction_threads",
										   "flusher_threads",
										   "compact_on_mount", "disableWAL" };
...
int RocksDBStore::load_rocksdb_options(bool create_if_missing,
									   rocksdb::Options &opt)
{
	rocksdb::Status status;
	...
	if (row_cache_size > 0)
		opt.row_cache = rocksdb::NewLRUCache(
			row_cache_size, g_conf()->rocksdb_cache_shard_bits);
	uint64_t bloom_bits =
		g_conf().get_val<uint64_t>("rocksdb_bloom_bits_per_key");
	if (bloom_bits > 0) {
	...
	bbt_opts.partition_filters =
		g_conf().get_val<bool>("rocksdb_partition_filters");
	if (g_conf().get_val<Option::size_t>("rocksdb_metadata_block_size") > 0)
		bbt_opts.metadata_block_size =
			g_conf().get_val<Option::size_t>("rocksdb_metadata_block_size");
	...
	dout(10) << __func__ << " block size " << g_conf()->rocksdb_block_size
			 ...
			 << (1 << g_conf()->rocksdb_cache_shard_bits) << ", type "
			 << g_conf()->rocksdb_cache_type << dendl;
...
int RocksDBStore::do_open(ostream &out, bool create_if_missing,
						  bool open_readonly, const vector<ColumnFamily> *cfs)
{
	...
	if (create_if_missing) {
		...
		status = rocksdb::DB::ListColumnFamilies(rocksdb::DBOptions(opt), path,
												 &existing_cfs);
		...
		if (existing_cfs.empty()) {
			...
			for (auto &n : existing_cfs) {
				...
				column_families.push_back(
					rocksdb::ColumnFamilyDescriptor(n, cf_opt));
			...
			if (open_readonly) {
				...
				status = rocksdb::DB::Open(rocksdb::DBOptions(opt), path,
			...
			for (unsigned i = 0; i < existing_cfs.size(); ++i) {
				if (existing_cfs[i] == rocksdb::kDefaultColumnFamilyName) {
					...
					add_column_family(existing_cfs[i],
	...
	PerfCountersBuilder plb(g_ceph_context, "rocksdb", l_rocksdb_first,
							l_rocksdb_last);
	plb.add_u64_counter(l_rocksdb_gets, "get", "Gets");
	plb.add_u64_counter(l_rocksdb_txns, "submit_transaction",
						"Submit transactions");
	plb.add_u64_counter(l_rocksdb_txns_sync, "submit_transaction_sync",
						"Submit transactions sync");
	plb.add_time_avg(l_rocksdb_get_latency, "get_latency", "Get latency");
	plb.add_time_avg(l_rocksdb_submit_latency, "submit_latency",
					 "Submit Latency");
	plb.add_time_avg(l_rocksdb_submit_sync_latency, "submit_sync_latency",
					 "Submit Sync Latency");
	plb.add_u64_counter(l_rocksdb_compact, "compact", "Compactions");
	plb.add_u64_counter(l_rocksdb_compact_range, "compact_range",
						"Compactions by range");
	plb.add_u64_counter(l_rocksdb_compact_queue_merge, "compact_queue_merge",
						"Mergings of ranges in compaction queue");
	plb.add_u64(l_rocksdb_compact_queue_len, "compact_queue_len",
				"Length of compaction queue");
	plb.add_time_avg(l_rocksdb_write_wal_time, "rocksdb_write_wal_time",
					 "Rocksdb write wal time");
	plb.add_time_avg(l_rocksdb_write_memtable_time,
					 "rocksdb_write_memtable_time",
					 "Rocksdb write memtable time");
	plb.add_time_avg(l_rocksdb_write_delay_time, "rocksdb_write_delay_time",
...
void RocksDBStore::close()
{
	...
	if (compact_thread.is_started()) {
		dout(1)
			<< __func__ << " waiting for compaction thread to stop" << dendl;
...
void RocksDBStore::split_stats(const std::string &s, char delim,
...
void RocksDBStore::get_statistics(Formatter *f)
{
	if (!g_conf()->rocksdb_perf) {
		dout(20) << __func__
				 << " RocksDB perf is disabled, can't probe for stats" << dendl;
		return;
	}
...
int RocksDBStore::submit_common(rocksdb::WriteOptions &woptions,
								KeyValueDB::Transaction t)
{
	...
	*_dout << " Rocksdb transaction: " << bat_txc.seen << dendl;
	...
	if (g_conf()->rocksdb_perf) {
		...
		logger->tinc(l_rocksdb_write_pre_and_post_process_time,
...
int RocksDBStore::submit_transaction(KeyValueDB::Transaction t)
{
	...
	// elapsed time is collected here, so no need to log inside submit_common()
...
void RocksDBStore::RocksDBTransactionImpl::set(const string &prefix,
											   const string &k,
											   const bufferlist &to_set_bl)
{
	// Tracing logs are recorded inside put_bat() function
...
void RocksDBStore::RocksDBTransactionImpl::merge(const string &prefix,
												 const string &k,
												 const bufferlist &to_set_bl)
{
	auto cf = db->get_cf_handle(prefix);
...
int RocksDBStore::get(const string &prefix, const std::set<string> &keys,
...
int RocksDBStore::get(const string &prefix, const char *key, size_t keylen,
					  bufferlist *out)
{
	ceph_assert(out && (out->length() == 0));
	utime_t start = ceph_clock_now();
...
int RocksDBStore::RocksDBWholeSpaceIteratorImpl::upper_bound(
	const string &prefix, const string &after)
{
...
int RocksDBStore::RocksDBWholeSpaceIteratorImpl::lower_bound(
	const string &prefix, const string &to)
{
...
bool RocksDBStore::RocksDBWholeSpaceIteratorImpl::raw_key_is_prefixed(
	const string &prefix)
{
...
public:
	explicit CFIteratorImpl(const std::string &p, rocksdb::Iterator *iter)
		: prefix(p)
		, dbiter(iter)
	{
	}
...
KeyValueDB::Iterator RocksDBStore::get_iterator(const std::string &prefix)
{
	...
	if (cf_handle) {
		return std::make_shared<CFIteratorImpl>(
			prefix, db->NewIterator(rocksdb::ReadOptions(), cf_handle));
	} else {
